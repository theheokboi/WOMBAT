...
from inframap.serve.redirects import register_ui_redirects
...
def create_app(
    ...
    def run_status(run_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            pass
        return build_run_status_payload(store, runtime_expectations, run_id, pointer=pointer, lane=lane)
    ...
    def runs_catalog(limit: int = Query(default=50, ge=1, le=200)) -> dict[str, Any]:
        ...
        for run_dir in run_dirs[:limit]:
            run_id = run_dir.name
            country_meta = latest_layer_metadata_for(run_dir, "country_mask") or {}
            adaptive_meta = latest_layer_metadata_for(run_dir, "facility_density_adaptive") or {}
    ...
    def list_layers(run_id: str | None = None) -> dict[str, Any]:
        ...
        metadata = load_layer_metadata(store.run_root(run_id))
    ...
    def get_layer_metadata(layer: str, run_id: str | None = None) -> dict[str, Any]:
        run_id = run_id or store.latest_run_id()
        candidates = [m for m in load_layer_metadata(store.run_root(run_id)) if m.get("layer_name") == layer]
        if not candidates:
