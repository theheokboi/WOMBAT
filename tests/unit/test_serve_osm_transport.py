...
def test_osm_transport_overlay_filters_mainline_classes_and_includes_railway(tmp_path: Path) -> None:
    ...
    assert "graph_variant" not in payload
    assert "run_agnostic" not in payload
...
def test_osm_transport_overlay_country_filter_rejects_unavailable_country(tmp_path: Path) -> None:
    osm_root = tmp_path / "openstreetmap"
    tw = osm_root / "TW"
    tw.mkdir(parents=True)
    _write_roads_shapefile(tw / "gis_osm_roads_free_1.shp")
    ...
    client = _build_client(tmp_path, osm_root)
    response = client.get("/v1/osm/transport?country=US")

    assert response.status_code == 404
    assert "OSM transport data unavailable for country" in response.json()["detail"]
